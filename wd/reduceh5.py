...
labels = {
    ...
    'eps_grav': 'eps_grav (ergs g-1 s-1)',
    'eps_nuc': 'eps_nuc (ergs g-1 s-1)', 
    'eps_neu': 'eps_neu (ergs g-1 s-1)',
    'eps_a': 'eps_a (ergs g-1 s-1)',
    
    'lum_gamma': 'lum_gamma (ergs s-1)',
    'lum_neu': 'lum_neu (ergs s-1)',
    'lum_a': 'lum_a (ergs s-1)',
    'num_a': 'num_a (s-1)',
    
    'lum_gamma_surf': 'surface lum_gamma (ergs s-1)',
    'lum_neu_surf': 'surface lum_neu (ergs s-1)',
    'lum_a_surf': 'surface lum_a (ergs s-1)',
    'num_a_surf': 'surface num_a (s-1)',
...
labels['lum_nuc'] = 'lum_nuc (ergs s-1)'
labels['lum_nuc_surf'] = 'surface lum_nuc (ergs s-1)'
...
for m in masses: 
    for g in couplings:
        ...
        p0 = profiles[0]
        with h5py.File(f'{outpath}/index.hdf5', 'w') as f:
            for key in labels:
                if np.isscalar(p0[key]):
                    f[labels[key]] = [ p[key] for p in profiles ]

        if not slices: 
            continue

        with h5py.File(f'{outpath}/slices.hdf5', 'w') as f:
            for p in profiles:
                group = f.create_group(f'slice_{p["model"]}')
                for key in labels:
                    if not np.isscalar(p0[key]):
                        group[labels[key]] = p[key]
